...
struct EditText{
	...
	bool isHover, isChoose;
	char** dataFill; //data entered by the user
	...
	void draw(){
		...
		bar(x+110, y, x+width, y+height);

		if(strlen(content) == 0){
			// Ve hint - phan text hint cua moi box
			setcolor(TEXT_EDIITEXT_HINT_COLOR);
			setbkcolor(BG_EDITTEXT_COLOR);
			outtextxy(x + 130, y+(height-textheight("H"))/2, hint);
		}else{
			// Ve noi dung nguoi dung nhap vao
			setcolor(TEXT_EDITTEXT_COLOR);
			setbkcolor(BG_EDITTEXT_COLOR);
			
			outtextxy(x + 130, y+(height-textheight("C"))/2, content);
