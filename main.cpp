...
void WriteDocGiaToFile(NodeDocGia* &TreeDG){
	...
	fileDocGia.open("data_docgia.txt", ios::out | ios::trunc); //ios::out : opent a file to write   \\ ios::trunc : if file is exit, so content of file wil cut before open fille
...
//	int n = atoi(line.c_str()); 
	int n;
	fileDocGia >> n;
	fileDocGia.ignore();  //https://codelearn.io/sharing/kieu-ky-tu-trong-cpp-la-gi-phan-2  => xoa ky tu dau tien trong bo nho dem
	int m;
	for(int i=0; i<n; i++){  
		DocGia dg; 
...
void DrawMenu(){    
	settextstyle(BOLD_FONT, HORIZ_DIR, BG_BUTTON_DEFAULT);
	setcolor(TEXT_TITLE_COLOR);  
	setbkcolor(BG_HOME_COLOR);      
	outtextxy(120, 45, "MENU");
	                   
	setcolor(LINE);            
	setlinestyle(SOLID_LINE, 0, 1);
	rectangle(5,5,400,h-100);    	//left frame
	rectangle(10,10,395,h-105); 	//left subframe
	
   	rectangle(405,5,w-10,h-100); 	//right frame
    rectangle(410,10,w-15,160); 	//top right subframe
    rectangle(410,170,w-15,h-105);	//bottom right subframe
...
void Event(){       
	mx = mousex(); my = mousey();  // return a x/y coordinate
	KbEvent(); 					   // take the characters from the keyboard and display them on the screen
